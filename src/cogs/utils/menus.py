# discord-ext-menus
# This Is Not The Correct Repo, Its Orignal Version Can Be Found Here : https://github.com/Rapptz/discord-ext-menus
...
class MenuPages(Menu):
    ...
    @button('<:doubleleft:820319249800101919>',
            position=First(0), skip_if=_skip_double_triangle_buttons)
    async def go_to_first_page(self, payload):
        """go to the first page"""
        await self.show_page(0)

    @button('<:left:820319249947689020>', position=First(1))
    async def go_to_previous_page(self, payload):
        """go to the previous page"""
        await self.show_checked_page(self.current_page - 1)

    @button('<:right:820319249766547456>', position=Last(0))
    async def go_to_next_page(self, payload):
        """go to the next page"""
        await self.show_checked_page(self.current_page + 1)

    @button('<:doubleright:820319249759338526>',
    ...
    @button('<:stop:820319249611620373>', position=Last(2))
