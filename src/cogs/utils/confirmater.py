...
class Dialog(ABC):
    ...
    async def update(self, title: str = None, description: str = None, color: hex = None, hide_author: bool = False):
        ...
        if title is not None:self._embed.title = title
        else:self._embed.title = None
        if description is not None:self._embed.description = description
        else:self._embed.description = None
        if hide_author:self._embed.set_author(name="")
