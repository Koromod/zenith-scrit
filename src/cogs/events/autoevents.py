    ...
    async def on_member_join(self, member):
        ...
        if not member.bot:
            if record['autorole_human_toggle'] is False:
                return
            try:
                await member.add_roles(discord.Object(id=record['autorole_human']), reason="Zenith's autorole")
            ...
            if record['autorole_bot_toggle'] is False:
                return
            try:
                await member.add_roles(discord.Object(id=record['autorole_bot']), reason="Zenith's autorole")
            except discord.Forbidden:
                return
        else:
            pass
