...
class Fun(commands.Cog, name='Fun'):
    ...
    async def meme(self, ctx):
        ...
        user = ctx.author
        req = requests.get(f"https://meme-api.herokuapp.com/gimme").json()
        foot = f"{req['ups']}👍 | Requested by {user} at {current_time} IST"
        mb = discord.Embed(description=f"[{req['title']}]({req['postLink']})",color=discord.Colour.green())
        mb.set_footer(text=foot)
        mb.set_image(url=req["url"])
