...
####################################################################################################################
    ...
    async def on_message(self,message):
        ...
        if not scrims:
        ...
        elif "zenith-smanager" in [role.name for role in message.author.roles]:
            # print('bot,role')
            return

        elif "zenith-sm-banned" in [role.name for role in message.author.roles]:
            ...
            if team_name in self.scrim_data[scrims['c_id']]["team_name"]:
                if scrims['auto_delete_on_reject'] == True:
                    self.bot.loop.create_task(delete_denied_message(message))
                    self.bot.dispatch("deny_reg",message,"already_registerd",scrims)
                    return
...
####################################################################################################################
    ...
    @commands.Cog.listener()
    async def on_deny_reg(self,message,type,scrims,addreact=True):
        if type == "insufficient_mentions":
            embed=discord.Embed(title="Insufficient mention", description=f"You did not mention required mention.\nRequired mention : {type} \nYour mention : {message.content} ",color=self.bot.color)
            try:
                member = message.author
                await member.send(embed=embed)
            except:
                await message.reply(embed=embed,delete_after=10)
            self.bot.dispatch("deny_reg_logs",f"Reagistration For {message.author}'s Team Has Been Denied Due To Insufficient Mentions",message.guild.id)
            if addreact == True:
                try:
            ...
            embed=discord.Embed(title="Bot detected", description="Yoy Mentioned A Bot",color=self.bot.color)
            try:
                member = message.author
                await member.send(embed=embed)
            except:
            ...
            embed=discord.Embed(title="Ban Team", description="Your team will be banned from scrims.",color=self.bot.color)
            try:
                member = message.author
                await member.send(embed=embed)
            except:
        ...
        #             pass
            ...
            embed=discord.Embed(title="Already registered", description=f"Your team is already registered with this team name {message.author}.",color=self.bot.color)
            try:
                member = message.author
                await member.send(embed=embed)
            except:
    ...
    async def on_correct_reg_logs(self,message,team_name):
        log_ch = discord.utils.get(message.guild.channels, name='zenith-sm-logs')
    ...
    async def on_reg_closed_logs(self,customid,customname,customnum,guild_id):
        ...
        log_channel = discord.utils.get(guild.channels, name='zenith-sm-logs')
    ...
    async def on_reg_open_msg_logs(self,ch_id,guild_id):
        ...
        data = await self.bot.db.fetchrow(f"SELECT * FROM smanager.custom_data WHERE reg_ch = $1",ch.id)
        log_ch = discord.utils.get(guild.channels, name='zenith-sm-logs')
        # print(log_ch)
        custom_id = data['c_id']
        ...
        if not data: pass
            ...
            if channel.id == int(data['reg_ch']):
                ...
                if data['is_running'] == True:
                    ...
                    pass
                log_channel = discord.utils.get(channel.guild.channels, name='zenith-sm-logs')
...
####################################################################################################################
    ...
    async def on_tag_check_message(self,message):
        ...
        if message.author.bot or "zenith-smanager" in [role.name for role in message.author.roles]:
            return
