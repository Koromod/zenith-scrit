...
class Esports(commands.Cog):
    ...
    async def smanager_setup(self,ctx):
        '''
        Setups The Zenith Scrims Manager In Your Server
        '''
        ...
        if scrims_manager is False:
            ...
            sm_role = await guild.create_role(name='zenith-smanager',permissions=permissions,colour=self.bot.color)
            sm_banned_role = await guild.create_role(name='zenith-sm-banned')
            guild = ctx.guild
            member = ctx.author
            overwrites = {
            ...
            smlogchannel = await guild.create_text_channel('zenith-sm-logs', overwrites=overwrites)
            try:
    ...
    @commands.has_role('zenith-smanager')
    # @is_smanager_setuped()
...
> 6. You Should Not Use Simple Text In Embed Builder.
    ...
    # @commands.has_role('zenith-smanger')
...
####################################################################################################################
    ...
    async def easytag_set(self,ctx,channel:discord.TextChannel):
        ...
        em = discord.Embed(title = "**__zenith Easy Tagging__**",color = self.bot.color)
        em.description = f"""
