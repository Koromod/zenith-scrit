...
class Owner_Commands(commands.Cog):
    ...
    async def brodcast(self,ctx,*,args):
        """
        Brodcasts This Message To Guild Having Zenith Setup
        """
        channel_id = []
        ...
        for channels in channel_id:
            ...
            start = time.time()
            em = discord.Embed(title = '📢 | ZenithES Announcement | 📢',description=args,color=self.bot.color)
            em.set_author(name="BuddyIsOp#6056",icon_url=ctx.author.avatar_url)
            try:
